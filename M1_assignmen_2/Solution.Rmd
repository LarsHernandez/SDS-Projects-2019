...
poke %>% group_by(Generation, Legendary) %>% summarize(N=n()) %>% ggplot(aes(Generation, N, fill=Legendary)) + 
  geom_col(width = 0.6) + 
  ...
  coord_flip() +
  labs(title="Pokemons by generation and Legendary status", subtitle="800 Pokemons in total")
```
...
ex <- km$centers %>% as_tibble %>% mutate(N=1:4) %>% gather(variable, value, -N)
...
ggplot(df, aes(PC1, PC2, col = cluster)) + 
  geom_point(data=df %>% select(-cluster), aes(PC1, PC2), inherit.aes = F, color="grey75", alpha=0.2, size=2) +
...
poke_scaled %>% mutate(Type1 = poke$Type1, 
  ...
  summarize(N=n()) %>% 
  ggplot(aes(Type1, N, fill=cluster)) + geom_col(position = position_dodge2(width = 0.9, preserve = "single")) + 
  scale_fill_tableau(palette = "Miller Stone", type = "regular") +
...
kable(head(poke_ml),digits = 2) %>% kable_styling(bootstrap_options = c("striped", "condensed"))
...
paste0("The training set has ", dim(training)[1], " And the test set has ", dim(test)[1], " observations")
```
...
### D. Models
*Fit three separate models on your training data, where you predict the “legendary” variable. Use a: *

1. Logistic regression
2. Decision tree, and
3. another algorithm of choice
...
kable(cbind(conf_glm, conf_log, conf_svm, conf_raf, conf_lda)) %>% 
