...
network <- c("Advice", "Friendship", "ReportsTo")

densities      <- c(edge_density(adv_n), edge_density(fri_n), edge_density(rep_n))
transitivities <- c(transitivity(adv_n), transitivity(fri_n), transitivity(rep_n))
reciprocities  <- c(reciprocity(adv_n),  reciprocity(fri_n),  reciprocity(rep_n))
...
r5 <- c(assortativity(fri_n, degree(fri_n, mode = "in"), directed = TRUE),  "POP",    "Friendship")
...
r10 <- c(assortativity(adv_n, degree(adv_n, mode = "in"), directed = TRUE),  "POP",   "Advice")
...
```{r}
atb_dep <- atb %>% group_by(DEPT) %>% summarize(ID=mean(ID),AGE = mean(AGE))

atb_g <- unique(atb$DEPT)
...
atb_g <- data.frame(ID=atb_g + 1)

fri_grp <- tbl_graph(nodes=as.data.frame(atb_dep), edges=fri_g)
fri_grp
fri_grp %>% 
  ggraph(layout = "nicely") + 
  geom_edge_link(aes(width=n)) + 
  geom_node_point(size=15, color="#1f78b4") + 
...
adv_grp <- tbl_graph(nodes=as.data.frame(atb_dep), edges=adv_g)
adv_grp
adv_grp %>% 
  ggraph(layout = "nicely") + 
  geom_edge_link(aes(width=n)) + 
...
#adv_n <- tbl_graph(nodes=atb_dep, edges=adv) %E>% filter(V3 == 1) 
#fri_n <- tbl_graph(nodes=atb_dep, edges=fri) %E>% filter(V3 == 1) 



#adv_n %N>% group_by(DEPT) %>% summarize(ID = 1:5) 


 
  #ggraph(layout = "nicely") + 
  #  geom_edge_link(alpha=0.4) + 
  #  geom_node_point(aes(color=as.factor(DEPT)), size = 4, alpha=0.8) +
...
a <- fri_n %N>% mutate(centrality_dgr = centrality_degree(mode="in")) %>% 
  ...
  th + theme(legend.position = "top", plot.title = element_text(size=16))
...
a <- ggplot() +
  ...
  th + theme(plot.title = element_text(size=25))
