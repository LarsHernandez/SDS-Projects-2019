...
fake_split <- fake %>% select(id) %>% initial_split()
train_data <- training(fake_split)
test_data  <- testing(fake_split)
...
model <- cv.glmnet(sparse_words, joined$labels, family = "multinomial", keep = T)
...
result <- table(res$class, res$labels)
result
accuracy(result)
```
...
model$glmnet.fit %>%
  ...
  ggplot(aes(reorder_within(term, estimate, class), estimate)) +
    ...
    scale_y_continuous(limits = c(-2,2), breaks = seq(-3, 3, by = 0.5)) + 
